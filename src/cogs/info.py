...
class Info(commands.Cog):
    ...
    async def membercount(self, ctx: commands.Context) -> None:
        ...
        embed = discord.Embed(
            title="Member Count", description=str(count or "Unknown"), color=EMBED_COLOR
        )
    ...
    async def channelinfo(
        self, ctx: commands.Context, channel: discord.abc.GuildChannel | None = None
    ) -> None:
        if ctx.guild is None:
            await send(
                ctx,
        ...
        embed = discord.Embed(
            title=f"Channel Info - {getattr(ch, 'name', 'Unknown')}", color=EMBED_COLOR
        )
        embed.add_field(name="ID", value=str(ch.id), inline=True)
        embed.add_field(
            name="Type",
            value=ch.type.name if hasattr(ch, "type") else type(ch).__name__,
            inline=True,
        )
        ...
        if isinstance(ch, discord.TextChannel):
            embed.add_field(
                name="NSFW", value="Yes" if ch.is_nsfw() else "No", inline=True
            )
            embed.add_field(name="Slowmode", value=f"{ch.slowmode_delay}s", inline=True)
            embed.add_field(
                name="Topic", value=(ch.topic or "None")[:1024], inline=False
            )
        elif isinstance(ch, discord.VoiceChannel):
            embed.add_field(name="Bitrate", value=str(ch.bitrate), inline=True)
            embed.add_field(
                name="User Limit", value=str(ch.user_limit or 0), inline=True
            )
